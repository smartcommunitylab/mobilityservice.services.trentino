...
//			mapper.writerWithDefaultPrettyPrinter().writeValue(new File("ordinanze.txt"), list);
			return Unirest.post("http://localhost:8080/core.mobility/servicedata/publishAlertRoads").header("Accept", "application/json").header("Content-Type", "application/json").body(list).asString().getStatus();
		} catch (Exception e) {
			throw new MobilityServiceException(e);
		}
